...
void writes(char *s) { mos_fwrite(1, s, strlen(s)); }
void wlog(char *s) { printf(s); }//mos_fwrite(2, s, strlen(s)); }
...
cell t3xseek(cell fd, cell where, cell how) {
	...
	return mos_flseek(fd, w, h) < 0? 0xffff: 0;
#else
	fail("t3x.seek not implemented");
	return 0;
#endif
}
...
cell libcall(cell n) {
	...
	case  7: r = fopen((char *) &M[w(P+2)], 'w'); break; // create, maybe wrong mode.
//	case  8: r = t3xopen((char *) &M[w(P+4)], w(P+2)); break;
	case  8: r = fopen((char *) &M[w(P+4)], 'r+'); break;
	case  9: r = fclose(w(P+2)); break;
	case 10: r = fread(&M[w(P+4)], 1, w(P+2), w(P+6)); break;
//	case 11: r = fwrite(&M[w(P+4)], 1, w(P+2), w(P+6)); break;
	case 11: r = fwrite(&M[w(P+4)], 1, w(P+2), stdout); break;
	case 12: r = t3xseek(w(P+6), w(P+4), w(P+2)); break;
	case 13: r = mos_ren((char *) &M[w(P+4)], (char *)&M[w(P+2)]); break;
	case 14: r = mos_del((char *) &M[w(P+2)]); break;
